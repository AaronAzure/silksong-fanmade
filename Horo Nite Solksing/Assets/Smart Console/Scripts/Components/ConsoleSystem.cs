...
namespace ED.SC.Components
{
	...
	public class ConsoleSystem : MonoBehaviour
    {
		...
		private Command FindCommandByText(string text) => 
            Command.All.Find(command => string.Equals(command.Name.ToLower(), text.ToLower()));
		...
		private object[] FindCommandTargets(Command command)
		{
			...
			{
				targets.Add(FindObjectOfType(command.MethodInfo.DeclaringType));
			}
			else if (command.TargetType == MonoTargetType.Active)
			{
				targets.AddRange(FindObjectsOfType(command.MethodInfo.DeclaringType, false));
			}
			else if (command.TargetType == MonoTargetType.All)
			{
				targets.AddRange(FindObjectsOfType(command.MethodInfo.DeclaringType, true));
			}
		...
		private void ExecuteCommandOnTargets(Command command, object[] targets, object[] parameters = null)
		{
			...
			for (int i = 0; i < targets.Length; i++)
			{
				...
				if (typeof(IEnumerator).IsAssignableFrom(command.MethodInfo.ReturnType))
				{

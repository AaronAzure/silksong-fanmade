...
namespace ED.SC.Components
{
	public class Autocompletor : MonoBehaviour
	{
		...
		{
			List<Command> commands = Command.Availables.FindAll(cmd => cmd.Name.StartsWith(startWith));
			...
			if (commands.Count > 0)
			{
				...
				{
					newAutocompletions.Add(new LogMessage(commands[i].Name, LogMessageType.Autocompletion, commands[i].MethodInfo.GetParameters()));
				}
			}
		...
		private void EnsureAutocompletionGeneration(string input)
		{
			...
			else if (m_CommandInput.Length < commandInput.Length)
			{
				...
				List<LogMessage> obsoleteAutocompletions = SmartConsole.AutocompletionLogMessages.FindAll(lm => !lm.Text.StartsWith(commandInput));
